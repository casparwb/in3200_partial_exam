...
# include <stdio.h>
# include <omp.h>
# include "PE_functions_casparwb_copy.h"
# include <time.h>
# include <math.h>
...
int main(int narg, char **argv){
  time_t start, stop;
  double t, epsilon = atof(argv[3]), d = atof(argv[2]);
  int z = 0, n = 3;
  ...
  t = omp_get_wtime();
  read_graph_file(infile, &val, &col_idx, &row_ptr, &dangling_idx, z, nodes, edges); // Initilize arrays with data from input file
  t = omp_get_wtime() - t;
